...
//            return emptyList()
    ...
    private fun generateAndClearConfigFiles(configsProviders: MutableMap<String, StyleMetaInfo>) {
        ...
        val fileCode = StringBuilder()

        fileCode.append("package $packageName\n\n")
            .append("import kotlin.Unit\n\n")
            .append(
                """
                    /**
                     * auto-generated by DaVinCi, do not modify
                     */
                    public object ${moduleName ?: ""}DaVinCiStylePreviewInjector {
                      /**
                       * register preview configurations
                       */
                      public fun register(): Unit {
                        registerConfigs()
                      }
        
                      private fun registerConfigs(): Unit {
                """.trimIndent()
            ).append("\n")
            .apply {
                if (preview) {
                    configsProviders.forEach {
                        this.append("   ").append(it.value.registerBlock()).append("\n")
                    }
                } else {
                    this.append("       //not preview mode, ignore").append("\n")
                }
            }
            .append(
                """
...
//        val fileSpec = FileSpec.get(packageName ?: "", daVinCiStylesSpec)
        ...
        thread(true) {
            ...
            file.appendText(fileCode.toString())
            file.close()
        }
