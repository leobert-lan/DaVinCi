...
//    private val providers: Multimap<String, Pair<String, KSFile>> = HashMultimap.create()
    ...
    private val styleProviders: MutableMap<String, StyleMeta> = hashMapOf()

    private val styleFactoryProviders: MutableMap<String, StyleMeta> = hashMapOf()
        ...
        resolver.getSymbolsWithAnnotation(DAVINCI_STYLE_NAME)
            ...
            .forEach { style ->
                ...
                if (styleProviders.containsKey(constName)) {
                    logE("duplicated style name:${styleName}, original register:${styleProviders[constName]?.clzNode}",
                        style)
                    return@forEach
                }

                styleProviders[constName] = StyleMeta(
                    constName = constName,
                    styleName = styleName,
                    clzNode = style
                )
    ...
    private fun handleDaVinCiStyleFactory(resolver: Resolver, notationType: KSType) {
        resolver.getSymbolsWithAnnotation(DAVINCI_STYLE_FACTORY_NAME)
            ...
            .forEach { style ->
                ...
                val constName = generateConstOfStyleName(styleName)
                if (styleProviders.containsKey(constName)) {
                    logE("duplicated style name:${styleName}, original register:${styleFactoryProviders[constName]?.clzNode}",
                        style)
                    return@forEach
                }

                styleFactoryProviders[constName] = StyleMeta(
                    constName = constName,
                    styleName = styleName,
                    clzNode = style
                )
    ...
    private fun generateConstOfStyleName(styleName: String): String =
        styleName.split(".").joinToString("_")


    private fun generateAndClearConfigFiles() {

        val properties = HashSet<StyleMeta>().apply {
...
public class StyleMeta(
    ...
    public fun propertySpec(): PropertySpec {
        return PropertySpec.builder(constName, String::class,
            arrayListOf(
